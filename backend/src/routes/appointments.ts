...
appointmentRouter.patch('/status', async(req: Request, res:Response) => {
  const {id} = req.params as { id: string };
  const {status} = req.body;

  const appointment = await db.update(appointmentSchema).set({ status }).where(eq(appointmentSchema.id, id)).returning();
  res.json(appointment)
});
